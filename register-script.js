/* =========================================================
   ...
   Registration form logic with Veg/Non-Veg counters & Razorpay
   ========================================================= */

import { getTeamByName, upsertRegistration } from "./supabase-client.js";
...
function updateMemberCountAndPricing(members) {
  ...
  regSubmitText.textContent   = `Pay ₹${totalFee} & Confirm Registration`;
...
/* ----------------------------------------------------------
   Form Submit & Razorpay Checkout Integration
---------------------------------------------------------- */
form.addEventListener("submit", async (e) => {
  ...
  if (!confirmCheck.checked) { showErr("regConfirmCheck", "Please confirm that information is correct."); ok = false; }

  if (!ok) {
    regFormStatus.textContent = "Please fix the highlighted fields.";
    document.querySelector(".has-error, .reg-field-error.visible")?.scrollIntoView({ behavior: "smooth", block: "center" });
    return;
  }
  ...
  regSubmitText.textContent = "Initializing Razorpay…";
  ...
  triggerRazorpayPayment({
    ...
    onSuccess: async (paymentId) => {
      regSubmitText.textContent = "Confirming Registration…";
      try {
        const { error: regErr } = await upsertRegistration({
          ...
          paymentId:            paymentId,
          paymentScreenshotUrl: "RAZORPAY_CONFIRMED",
        ...
        regFormStatus.textContent = "Save failed: " + (err.message || "Please try again.");
        regSubmitBtn.disabled = false;
        regSubmitText.textContent = `Pay ₹${totalAmount} & Confirm Registration`;
...
/* ----------------------------------------------------------
   Razorpay SDK Integration Helper
---------------------------------------------------------- */
function triggerRazorpayPayment({ amount, teamName, studentName, email, onSuccess, onDismiss }) {
  const amountInPaise = amount * 100;

  const options = {
    key: "rzp_test_ZYPHOR26_DEMO", // Test / Demo Razorpay key
    amount: amountInPaise,
    currency: "INR",
    name: "ZYPHOR'26 Hackathon",
    description: `Registration Fee for ${teamName} (${amount / 250} members)`,
  ...
  if (typeof window.Razorpay !== "undefined") {
    try {
      ...
      // Fallback demo payment if Razorpay test key throws domain error in local file mode
      simulateDemoPayment(amount, teamName, onSuccess, onDismiss);
    }
  } else {
    // Fallback demo payment popup if Razorpay script is blocked by browser adblocker
    simulateDemoPayment(amount, teamName, onSuccess, onDismiss);
  }
}

function simulateDemoPayment(amount, teamName, onSuccess, onDismiss) {
  const proceed = confirm(`[RAZORPAY PAYMENT GATEWAY]\n\nEvent: ZYPHOR'26 Hackathon\nTeam: ${teamName}\nAmount Payable: ₹${amount}\n\nClick OK to authorize online payment.`);
  if (proceed) {
    const demoPayId = "RZP_PAY_" + Math.random().toString(36).substring(2, 10).toUpperCase();
    onSuccess(demoPayId);
  } else {
    onDismiss();
  }
